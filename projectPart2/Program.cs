        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                else if (choice == 8)
                {
                    if (cartCount == 0)
                    {
                        Console.WriteLine("INFO: Cart is empty. Nothing to checkout.");
                    }
                    else
                    {
                ...
                {
                    string optionToExit = PromptValidator("Are you sure you want to exit? (Y/N): ");
                    if (optionToExit == "Y")
                    {
                        if (historyCount > 0)
                        {
                            string viewHistory = PromptValidator("Do you want to view order history before exiting? (Y/N): ");
                            if (viewHistory == "Y")
                            {
                                DisplayOrderHistory();
                            }
            ...
            static int ShoppingMenu(Product[] products, CartItem[] cart, int cartCount)
            {
                ...
                while (true)
                {
                    ...
                    Console.WriteLine("ERROR: Invalid choice. Please enter a number between 1 and 9.");
                }
            }
